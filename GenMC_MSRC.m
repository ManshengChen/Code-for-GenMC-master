function [My_result]=GenMC_MSRC(X,gt,lambda,beta)
...
while(Isconverg == 0)
    ...
    for k=1:K
       ...
       %% Update E^k
        F = [X{1}-X{1}*Z{1}+Y{1}/mu;X{2}-X{2}*Z{2}+Y{2}/mu;X{3}-X{3}*Z{3}+Y{3}/mu;X{4}-X{4}*Z{4}+Y{4}/mu];
        ...
        E{4} = Econcat(size(X{1},1)+size(X{2},1)+size(X{3},1)+1:size(X{1},1)+size(X{2},1)+size(X{3},1)+size(X{4},1),:);
    ...
    [result(num,:)] = ClusteringMeasure1(gt, C);
end

My_result = result(num,:); %ACC MIhat F AR